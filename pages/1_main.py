...
from db.db import get_db_connection, create_tables
...
def load_session_state():
  if os.path.exists('session_state.pkl'):
    try:
      ...
      st.error("Error during loading session state")
  else:
    return None
...
def show_search_results(query=None):
  ...
  if query:
    filtered_listings = [
        listing for listing in listings if query.lower() in listing[1].lower()
    ]
  else:
